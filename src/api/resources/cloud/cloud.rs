...
impl CloudClient {
    ...
    pub async fn add_device(&self, entry: &String, request: &DeviceEntry, options: Option<RequestOptions>) -> Result<AddDeviceResponse, ApiError> {
    ...
    ///
    /// JSON response from the API
    pub async fn history_device(&self, entry: &String, device_id: &String, options: Option<RequestOptions>) -> Result<CloudQueryApiResponse, ApiError> {
        self.http_client.execute_request(
            Method::GET,
    ...
    ///
    /// JSON response from the API
    pub async fn list_device(&self, entry: &String, request: &ListDeviceQueryRequest, options: Option<RequestOptions>) -> Result<CloudQueryApiResponse, ApiError> {
        self.http_client.execute_request(
            Method::GET,
            &format!("Cloud/list/{}", entry),
            None,
            QueryBuilder::new().bool("forceRefresh", request.force_refresh.clone())
            .build(),
            options,
        ).await
    }
    ...
    ///
    /// JSON response from the API
    pub async fn remove_device(&self, entry: &String, device_id: &String, options: Option<RequestOptions>) -> Result<RemoveDeviceResponse, ApiError> {
        self.http_client.execute_request(
            Method::DELETE,
