...
impl TokenStorageClient {
    ...
    pub async fn add_method(&self, request: &AddMethodRequest, options: Option<RequestOptions>) -> Result<AddMethodResponse, ApiError> {
    ...
    ///
    /// JSON response from the API
    pub async fn get_method(&self, method_id: &String, request: &GetMethodQueryRequest, options: Option<RequestOptions>) -> Result<GetMethodResponse, ApiError> {
        self.http_client.execute_request(
            Method::GET,
            &format!("TokenStorage/{}", method_id),
            None,
            QueryBuilder::new().int("cardExpirationFormat", request.card_expiration_format.clone()).bool("includeTemporary", request.include_temporary.clone())
            .build(),
            options,
        ).await
    }
    ...
    ///
    /// JSON response from the API
    pub async fn remove_method(&self, method_id: &String, options: Option<RequestOptions>) -> Result<PayabliApiResponsePaymethodDelete, ApiError> {
        self.http_client.execute_request(
            Method::DELETE,
            &format!("TokenStorage/{}", method_id),
            None,
    ...
    ///
    /// JSON response from the API
    pub async fn update_method(&self, method_id: &String, request: &UpdateMethodRequest, options: Option<RequestOptions>) -> Result<PayabliApiResponsePaymethodDelete, ApiError> {
        self.http_client.execute_request(
            Method::PUT,
            &format!("TokenStorage/{}", method_id),
            Some(serde_json::to_value(&request.body).unwrap_or_default()),
            QueryBuilder::new().serialize("achValidation", request.ach_validation.clone())
            .build(),
            options,
        ).await
    }
